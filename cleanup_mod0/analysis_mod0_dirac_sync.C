...
#include "analysis_mod0_dirac_sync.h"
//#include <TH2.h>
//#include <TStyle.h>
//#include <TCanvas.h>
//#include <string>
...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   int previous_files_total_daq_events = 0;
   int previous_files_total_entries = 0;
   std::list<int>::iterator entries_it = entries_list.begin(); 
   ...
   for( Long64_t jentry=0; jentry<nentries;jentry++ ) { //nentries
     ...
     evnum = nevt + previous_files_total_daq_events;

     if (jentry - previous_files_total_entries == *entries_it-1){
       previous_files_total_daq_events += nevt+1;
       previous_files_total_entries += *entries_it;
       entries_it++;
     }
     ...
     */
     ...
     for( int jHit=0; jHit<nhits; jHit++ ){
       ...
       if (ChaTmp==14){
         ...
         for(int i=0; i<nofsamples[jHit]-1; i++){
           int t = timens[i], y = ADC[i];
           if (t < 450 || t > 650) continue;

           if(y > 2500 && t1 == -999) t1 = t;
           if(y > 2550 && t2 == -999){
             t2 = -555;
           }
            if(y < 2550 && t2 == -555){
         ...
         for(int i=nofsamples[jHit]-1; i>0; i--){
           int t = timens[i], y = ADC[i];
           if (t < 650 || t > 1000) continue;

           if(y > 2550 && t4 == -999) t4 = t;
           if(y < 2550 && t4!=-999){
             t3 = t;
             break;
           }
         ...
         if(t2 >= 475 && t2 < 595) crtBarTop = (t2 - 475)/30 * 2;
         if(t4 - t3 >= 20 && t4-t3 < 160) crtBarBot = (t4 - t3 - 20)/30 * 2 - (t4 - t3 >= 140)*2;
...
int main(){
  ...
  analysis_mod0 *a = new analysis_mod0("run165list.txt");

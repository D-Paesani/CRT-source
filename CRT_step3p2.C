...
double **timeOffset, **zetaOffset, **chargeEqual, **chargeEqualErr, **chargeEqual_out, **chargeEqualErr_out;
...
void zetaMip_proc(TH1* histObj, int histN, int& histSkipFlag) {   
  ...
  if (centerMode) {
    ...
    zFit = TF1("l", "( TMath::TanH( (x-[0])/[1] ) - TMath::TanH( (x-[2]) /[1] ) ) * ([3]+[4]*x)", -1.2*scintL, 1.2*scintL);
...
void Analysis::ProcessPlots() {
  ...
  //ChEqOld
    chEqGraph = new TGraphErrors(2*scintNum); chEqGraph->SetTitle("chargeEqual_s3p1");
    qMean = 0;
    for (int k = 0; k < scintNum; k++) { 
    ...
    qMean = qMean/(2*scintNum);
    line = new TLine(0.5, qMean, scintNum+0.5, qMean); line->SetLineColor(kRed);
  ...
  //TimeOff
    // TGraphErrors *tim = new TGraphErrors(2*scintNum); 
    // for (int k = 0; k < scintNum; k++) { 
...
void Analysis::LoopOverEntries() {
  ...
  Long64_t etp = min(maxEvToProcess3p2, nentries);
...
void Analysis::Loop(){
  ...
  cout<<endl<<endl<<"::::::::::::::::::::: CRT analysis step 3p2 :::::::::::::::::::::"<<endl<<endl;
  ...
  //CSV.Write(   lutPrefix3p + runName + lutChEqName         + ".csv", ',', chargeEqual_out, 2, scintNum, 4);
  //CSV.Write(   lutPrefix3p + runName + lutTimeOffsName     + ".csv", ',', timeOffset,      2, scintNum, 4);
  CSV.Write(   lutPrefix3p + runName + lutZetaOffName      + ".csv", ',', zetaOffset,      1, scintNum, 4);
  cout<<"...done"<<endl;

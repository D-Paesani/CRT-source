   ...
   for( Long64_t jentry=0; jentry<nentries;jentry++ ) { //nentries
     ...
     */
     ...
     for( int jHit=0; jHit<nhits; jHit++ ){
       ...
       if (ChaTmp==14 && ADC[peakpos[jHit]] < 2500){ // asks only one mip
         int lasttimens = timens[firstsample[jHit] + nofsamples[jHit] -1 ];
         crtBar = (lasttimens -375)/15;
       }
...
void analysis_mod0::BookOutput(TTree *mod0)
{
  ...
  mod0->Branch("crtBar"    ,&crtBar,    "crtBar/I");

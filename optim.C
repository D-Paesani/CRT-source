...
//Pars
  ...
  int constant_fraction_counter = 0;
...
void times_proc(TH1* histObj, int histN, int& histSkipFlag) { 
   ...
   histObj->Fit(&timeFit, "R");
   resolution[constant_fraction_counter] = timeFit.GetParameter(2);
...
int main(int argc, char*argv[]) { 
  ...
  ofstream of("reso.txt");
  for(int i=0; i<20; i++){
    constant_fraction_counter = i;
    constant_fraction_current = 0.05 + i*0.02;
    Analysis::Run(argv1, argv2, argv3, argv4, -1);
    of << constant_fraction_current << " " << resolution[constant_fraction_counter] << endl;
  }
  of.close();
}
